...
def obtener_vista_retiro(page: ft.Page, sesion: dict, al_volver):
    ...
    def mostrar_exito(monto):
        pantalla_exito = ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Text("¡Retiro realizado!", size=36, color=ft.Colors.WHITE,
    ...
    def confirmar(e):
        ...
        if monto > saldo_actual:
            error_msg.value = f"Saldo insuficiente. Tienes ${saldo_actual:,.2f} disponibles."
            error_msg.visible = True
            page.update()
            return

        db.registrar_transaccion(sesion["id"], "Retiro", monto, destino=sesion["num_cuenta"])
        mostrar_exito(monto)
    ...
    tarjeta = ft.Container(
        ...
        content=ft.Column([
            ft.Container(
                content=ft.Icon(ft.Icons.MONETIZATION_ON, size=48, color=ft.Colors.WHITE),
                alignment=ft.Alignment(0, 0),
            ),
            ft.Container(height=8),
            ft.Text("Retiro", size=26, color=ft.Colors.WHITE,
                    weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.CENTER),
            ...
            ft.Text(f"${db.obtener_saldo(sesion['id']):,.2f}", size=22, color=ft.Colors.WHITE,
                    weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.CENTER),
            ft.Divider(color=ft.Colors.with_opacity(0.2, ft.Colors.WHITE), height=30),
            ft.Text("Monto a retirar", color=ft.Colors.WHITE70, size=15),
        ...
        page.add(
            ft.Container(
                ...
                content=ft.Container(
                    ...
                    content=obtener_vista_retiro(page, sesion, al_volver=lambda: None),
                ),
            )
        )
